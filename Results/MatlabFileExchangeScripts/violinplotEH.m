%Purpose:
%Violin plot, with an input specifying position of violin along the x-axis

function violins = violinplotEH(data, cats, XPosViol,XPosLabels, varargin)
...
%   DATA.
%
%   VIOLINPLOT(DATA, CATEGORIES,XPos) where double vector DATA and vector
%   CATEGORIES are of equal length; plots violins for each category in
%   DATA at position XPos
%
...
% This code is released under the terms of the BSD 3-clause license
    ...
    if isa(data, 'dataset') || isstruct(data) || istable(data)
        ...
        for n=1:length(catnames)
            ...
            violins(n) = Violin(thisData, XPosViol(n), varargin{:});
        end
        set(gca, 'xtick', XPosLabels, 'xticklabels', catnames);
