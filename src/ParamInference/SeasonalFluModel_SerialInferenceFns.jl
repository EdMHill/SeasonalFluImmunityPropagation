...
function  OLCMPerturbVarFn(RetainedParams,RetainedWeights,SurvivorParticleIdx,FirstGenFlag)
...
function  SampleFirstGenFn_FromFile(N::Int64)
#Inputs:
...
#season
function  SampleFirstGenFn_FourSeasonFit(N)
#Inputs:
...
#Draw samples from specified prior ranges, with ascertainment prob. per
#season
function  SampleFirstGenFn_FiveSeasonFit(N)
#Inputs:
#   N - Number of samples from prior to take forward as first generation
...
#Draw samples from specified prior ranges, with ascertainment prob. per
#season
function  SampleFirstGenFn_SixSeasonFit(N)
#Inputs:
#   N - Number of samples from prior to take forward as first generation
...
function  SampleFirstGenFn_SixSeasonFitPlusMultiSeasonImm(N)
#Inputs:
#   N - Number of samples from prior to take forward as first generation
...
#seasons inclusive
function Prior_FourSeasonFit(x)
...
#seasons inclusive
function function Prior_FiveSeasonFit(x)
...
#seasons inclusive
#Includes parameter for carry-over immunity resulting from natural infection
function Prior_SixSeasonFitPlusMultiSeasonImm(x)
...
#-------------------------------------------------------------------------------
function  RunModelSimn(FixedModelParams,x)
#Inputs:
...
function  RunModelSimnWithMultiSeasonImm(FixedModelParams,x)
#Inputs:
#   FixedModelParams - Parameters with consistent values across all runs
#   x - Values of parameters that are being inferred
...
function BuildExpHistArray(NumOfStrains,ExpHistArrayParams)
